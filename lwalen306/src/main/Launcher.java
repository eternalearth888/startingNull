...
public class Launcher {
	...
	public Pokeball launch() {
		Pokeball p = new Pokeball(Math.cos(Math.toRadians(angle))*104, Math.sin(Math.toRadians(angle))*104, Math.cos(Math.toRadians(angle))*power, Math.sin(Math.toRadians(angle))*power);
		return p;
	}
