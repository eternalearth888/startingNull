...
public class SaveTheCake extends JPanel {
	...
	public SaveTheCake() {
		...
		ball = new Ball(FRAME_SIZE_X - OFFSET - 15, OFFSET - 40, OFFSET,
				Color.GREEN);
		...
		while (true) { // 11/30/2012
			while (!cake.intersect(ball)) { // When the game is not done
				...
				for (Bullet i : bullets) { // In each cycle,
					...
					if (i.intersect(ball)) { // 11/30 // if bullet hits ball
												// then ball explodes
						ball.setDiameter(ball.getDiameter()
								- BALL_DIA_REDUCTION);
						ball.explode();
						if (ball.getDiameter() < 1) { // When ball is destroyed
														// => win
			...
			restart = JOptionPane.showConfirmDialog(null,
					"Do you want to shoot again?", "Welcome to Save The Cake",
					JOptionPane.YES_NO_OPTION);
			if (restart == JOptionPane.YES_OPTION) {
				ball = new Ball(FRAME_SIZE_X - OFFSET - 15, OFFSET - 40,
						OFFSET, Color.GREEN);
				cake.reset();
			} else {
